...
public abstract class Enchant extends Enchantment implements Listener {
    public Enchant(@NotNull String key) {
        super(new NamespacedKey(JavaPlugin.getPlugin(EnchantmentsGalore.class), key));
        JavaPlugin mainPlugin = JavaPlugin.getPlugin(EnchantmentsGalore.class);
        if (getByKey(new NamespacedKey(JavaPlugin.getPlugin(EnchantmentsGalore.class), key)) == null) {
            try {
